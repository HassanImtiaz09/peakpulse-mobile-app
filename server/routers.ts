  ...
  mealPlan: router({
    ...
    generate: guestOrUserProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const prompt = `Generate a 7-day meal plan as JSON. Goal: ${input.goal.replace(/_/g," ")}, Diet: ${input.dietaryPreference}, Calories: ${calories}kcal. Return this exact structure: {"dailyCalories":${calories},"proteinTarget":150,"carbTarget":200,"fatTarget":65,"dietType":"${input.dietaryPreference}","days":[{"day":"Monday","meals":[{"name":"Scrambled Eggs with Spinach","type":"breakfast","calories":420,"protein":28,"carbs":35,"fat":14,"ingredients":["3 large eggs","1 cup fresh spinach","1 tbsp olive oil","salt and pepper"],"prepTime":"10 min","instructions":["Heat olive oil in a pan over medium heat","Add spinach and wilt for 1 minute","Whisk eggs and pour over spinach","Scramble gently until just set","Season with salt and pepper"],"photoQuery":"scrambled eggs spinach breakfast"}]}],"insight":"personalized nutrition tip"}. Include 4-5 meals per day (breakfast, morning snack, lunch, afternoon snack, dinner). Each meal MUST have ingredients array, instructions array (3-5 steps), and photoQuery (2-4 word food search term). Respect dietary restrictions strictly.`;
