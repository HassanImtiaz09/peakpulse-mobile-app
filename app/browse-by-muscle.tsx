/**
 * Browse by Muscle — Discovery Screen
 *
 * Tappable anatomical muscle diagram that lets users explore exercises
 * by muscle group. Tap a muscle region to see all exercises targeting it,
 * with ExerciseDB animated GIF previews.
 */
...
// ── Tappable SVG Paths ──────────────────────────────────────────────────────
// Same paths as muscle-svg-diagram but with touch regions
...
const BACK_REGIONS: MuscleRegion[] = [
  {
    muscle: "traps",
    label: "Traps",
    paths: ["M30,30 L58,30 L54,38 L34,38 Z"],
  },
  {
    muscle: "shoulders",
    label: "Rear Delts",
  ...
  },
  {
    muscle: "back",
    label: "Upper Back",
    paths: ["M34,38 L54,38 L52,52 L36,52 Z"],
  },
  {
    muscle: "lower_back",
    label: "Lower Back",
    paths: ["M36,52 L52,52 L50,68 L38,68 Z"],
  },
  {
    muscle: "triceps",
    label: "Triceps",
...
// Head/neck decorative paths (not tappable)
const DECO_PATHS = [
...
export default function BrowseByMuscleScreen() {
  ...
  const handleExercisePress = useCallback(
    (exercise: ExerciseInfo) => {
      ...
      // Navigate to exercise detail if the screen exists
      router.push({
        ...
        <View style={styles.toggleRow}>
          ...
          >
            <Text
              style={[
                styles.toggleTxt,
                side === "front" && styles.toggleTxtActive,
              ]}
            >
              FRONT
            </Text>
          ...
          >
            <Text
              style={[
                styles.toggleTxt,
                side === "back" && styles.toggleTxtActive,
              ]}
            >
              BACK
            </Text>
...
// ── Label Positioning ───────────────────────────────────────────────────────
// Position labels around the body diagram based on muscle group

function getRegionLabelPosition(
  muscle: MuscleGroup,
  side: "front" | "back"
): { top?: number; left?: number; right?: number; bottom?: number } {
  // Positions are relative to the diagramWrap container
