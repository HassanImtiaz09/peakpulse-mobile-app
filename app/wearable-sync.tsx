...
export default function WearableSyncScreen() {
  ...
  const {
    stats, syncFromDevice, syncFromHealthPlatform, requestPermissions,
    isConnected: hasWearableData, getWeeklyAverage, syncing,
    healthSource, permissionStatus, healthSourceName, isHealthPlatformAvailable: nativeAvailable,
  } = useWearable();
  ...
  async function handleRefreshHealthData() {
    ...
    if (success) {
      ...
      Alert.alert("No Data", "Could not fetch health data. Make sure your wearable has synced to the health platform.");
    }
  }
  ...
  async function handleOpenApp(wearable: typeof THIRD_PARTY_WEARABLES[0]) {
    ...
    setOpening(null);
    Alert.alert("Synced", `${wearable.name} data synced. Ensure it writes to ${healthSourceName} for automatic updates.`);
  }
  ...
  const visibleWearables = THIRD_PARTY_WEARABLES.filter(w => platformFilter.includes(w.platform));

  const isHealthConnected = permissionStatus === "granted";
  const dataSourceLabel = stats.dataSource === "healthkit" ? "Apple Health"
    : stats.dataSource === "healthconnect" ? "Health Connect"
    : stats.dataSource === "simulated" ? "Simulated"
    ...
    <View style={{ flex: 1, backgroundColor: "#0A0500" }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{
          ...
          marginBottom: 16,
          borderWidth: 1.5,
          borderColor: isHealthConnected ? "rgba(34,197,94,0.30)" : "rgba(245,158,11,0.20)",
        }}>
          {/* Header */}
          <View style={{ flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 14 }}>
            <View style={{
              width: 52, height: 52, borderRadius: 16,
              backgroundColor: isHealthConnected ? "rgba(34,197,94,0.15)" : "rgba(245,158,11,0.15)",
              alignItems: "center", justifyContent: "center",
              borderWidth: 1, borderColor: isHealthConnected ? "rgba(34,197,94,0.30)" : "rgba(245,158,11,0.30)",
            }}>
              <MaterialIcons
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={{ color: "#B45309", fontSize: 12, lineHeight: 16 }}>
                {nativeAvailable
                  ...
                  : "Native health platform not available on web. Use simulated data or connect a third-party app."}
              </Text>
            </View>
          </View>
          ...
          <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginBottom: 14 }}>
            {["Steps", "Heart Rate", "Calories", "Sleep", "HRV", "VO2 Max", "SpO2", "Distance"].map(dt => (
              <View key={dt} style={{
                backgroundColor: isHealthConnected ? "rgba(34,197,94,0.08)" : "rgba(245,158,11,0.08)",
                borderRadius: 8, paddingHorizontal: 10, paddingVertical: 4,
                borderWidth: 1, borderColor: isHealthConnected ? "rgba(34,197,94,0.20)" : "rgba(245,158,11,0.15)",
              }}>
                <Text style={{ color: isHealthConnected ? "#22C55E" : "#B45309", fontSize: 11, fontFamily: "DMSans_600SemiBold" }}>{dt}</Text>
              </View>
            ))}
          </View>
          ...
          {nativeAvailable ? (
            isHealthConnected ? (
              <View style={{ flexDirection: "row", gap: 10 }}>
                <TouchableOpacity
                  style={{
                    flex: 2, backgroundColor: "#22C55E", borderRadius: 14, paddingVertical: 14,
                ...
                <TouchableOpacity
                  style={{
                    flex: 1, backgroundColor: "rgba(245,158,11,0.10)", borderRadius: 14, paddingVertical: 14,
                    alignItems: "center", borderWidth: 1, borderColor: "rgba(245,158,11,0.20)",
                  }}
                  onPress={() => {
                    if (Platform.OS === "ios") {
            ...
            <View style={{ backgroundColor: "rgba(245,158,11,0.08)", borderRadius: 12, padding: 12 }}>
              <Text style={{ color: "#B45309", fontSize: 12, textAlign: "center" }}>
                Health platform integration requires a native device (iPhone or Android). On web, data is simulated for preview.
              </Text>
            </View>
          )}
        </View>
        ...
        {hasWearableData && (
          <View style={{ backgroundColor: "#150A00", borderRadius: 20, padding: 18, marginBottom: 16, borderWidth: 1.5, borderColor: "rgba(245,158,11,0.20)" }}>
            ...
            {stats.lastSyncedAt && (
              <Text style={{ color: "#B45309", fontFamily: "DMSans_400Regular", fontSize: 10, marginBottom: 12 }}>
          ...
          return (
            <View
              ...
              style={{
                ...
                marginBottom: 12,
                borderWidth: 1,
                borderColor: isLinked ? wearable.color + "50" : isExpanded ? wearable.color + "30" : "rgba(245,158,11,0.10)",
                overflow: "hidden",
              }}
            >
              {/* Main Row */}
              ...
              >
                <View style={{ width: 44, height: 44, borderRadius: 14, backgroundColor: wearable.color + "20", alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: wearable.color + "40" }}>
                  <MaterialIcons name={wearable.icon} size={22} color={wearable.color} />
                </View>
                <View style={{ flex: 1 }}>
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 2 }}>
                  ...
                  <Text style={{ color: "#B45309", fontSize: 11, lineHeight: 15 }}>{wearable.description}</Text>
                </View>
                <MaterialIcons name={isExpanded ? "expand-less" : "expand-more"} size={20} color="#B45309" />
              </TouchableOpacity>
              ...
              {isExpanded && (
                <View style={{ paddingHorizontal: 16, paddingBottom: 16, gap: 12 }}>
                  ...
                  {isLinked ? (
                    <View style={{ flexDirection: "row", gap: 10 }}>
                      <TouchableOpacity
                        style={{ flex: 2, backgroundColor: wearable.color, borderRadius: 14, paddingVertical: 12, alignItems: "center", flexDirection: "row", justifyContent: "center", gap: 8 }}
                    ...
                    <TouchableOpacity
                      style={{
                        backgroundColor: wearable.color, borderRadius: 14, paddingVertical: 12,
                        alignItems: "center", flexDirection: "row", justifyContent: "center", gap: 8,
