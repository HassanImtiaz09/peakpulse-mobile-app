...
function QuickActionCard({ icon, label, onPress }: { icon: string; label: string; onPress: () => void }) {
  return (
    <TouchableOpacity style={styles.qaCard} onPress={onPress} activeOpacity={0.7}>
      <View style={styles.qaIconBox}><Text style={{ fontSize: 22 }}>{icon}</Text></View>
...
export default function HomeScreen() {
  ...
  // For guests: use AsyncStorage flag (local-only session; acceptable).
  useEffect(() => {
    if (authLoading || guestLoading) return;
    if (isAuthenticated) {
      // profile query resolves quickly; undefined means loading, null means no profile yet
      // We use a short timer to allow the query to settle before deciding
      const timer = setTimeout(() => setOnboardingChecked(true), 800);
      return () => clearTimeout(timer);
    }
    // Guest flow: local onboarding flag
  ...
  // For authenticated users without a profile, redirect to onboarding
  useEffect(() => {
    if (!isAuthenticated || authLoading) return;
    // profile === null means the query returned and there is no profile row yet
    if (profile === null) {
      router.replace("/onboarding" as any);
  ...
  // ── Stats (server-driven for auth users; AsyncStorage for guests) ────
  // For authenticated users, stats are derived from tRPC data so they sync across devices.
  // workoutCount = total exercises completed across all workout days (proxy: schedule days)
  // streak = not yet tracked server-side; kept at 0 until a dedicated endpoint is added
  // photos = total progress photos uploaded (from trpc.progress.getAll)
    ...
    <>
      ...
      <View style={{ flex: 1, backgroundColor: SF.bg }}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }} style={{ flex: 1 }}>
          {/* ── Hero Header ── */}
          <ImageBackground source={{ uri: AT_DASHBOARD_BG }} style={{ width: "100%", height: 300 }} resizeMode="cover">
            <View style={styles.heroOverlay}>
              <View style={styles.heroTopBar}>
                <View style={styles.heroBadge}><Text style={styles.heroBadgeText}>⚡ PEAKPULSE AI</Text></View>
                ...
                {(activeProfile as any)?.goal && (
                  <View style={{ flexDirection: "row", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
                    ...
                    {(activeProfile as any)?.workoutStyle && (
                      <View style={styles.heroPill}>
                        <Text style={styles.heroPillText}>🏋️ {(activeProfile as any).workoutStyle}</Text>
                      </View>
                    )}
                  </View>
                )}
